import hDate, {jewishMonthDaysCount, jewishIsLeapt} from 'hebrew-date'
import E from './holidaysCharacteristicsConst';

const isMonthLeapt = (year, month) => {
    return jewishMonthDaysCount(year, 10) > 29 
}

const holidays = {
    1: /* nisan */ {
        12: E.BDIKAT_CHAMETZH,
        13: [E.SOF_ZMAN_ACHILAT_CHAMETZH, E.ZMAN_BEOUR_CHAMETZH],
        14: E.EREV_PESACH_1,
        15: E.PESACH_1,
        16: (date, diaspora) => (diaspora ? E.PEASACH_1_YT2_GALOUYOT : E.CHOL_HAMOED_PESACH),
        17: E.CHOL_HAMOED_PESACH,
        18: E.CHOL_HAMOED_PESACH,
        19: E.CHOL_HAMOED_PESACH,
        20: [E.CHOL_HAMOED_PESACH, E.EREV_PESACH_7],
        21: E.PESACH_7,        
        21: (date, diaspora) => (diaspora ? E.PEASACH_7_YT2_GALOUYOT : [E.DAY_AFTER_PESACH_7]),
    ...
    12: /* Adar / Adar 1 */ {
        11: date => (jewishIsLeapt(date.jewishToJSON().year) ? undefined : () => holidays[13][11]), // TODO: brought forward
        13: date => (jewishIsLeapt(date.jewishToJSON().year) ? 'erevPurimKatan': () => holidays[13][13]),
        14: date => (jewishIsLeapt(date.jewishToJSON().year) ? 'purimKatan' : () => holidays[13][14]),
        15: date => (jewishIsLeapt(date.jewishToJSON().year) ? 'shushanPurimKatan' :  () => holidays[13][15]),
    },
    13: /* Adar 2 */ {
